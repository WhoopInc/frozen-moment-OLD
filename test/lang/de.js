...
exports["lang:de"] = {
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "ein paar Sekunden",  "44 seconds = a few seconds");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  "eine Minute",       "45 seconds = a minute");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  "eine Minute",       "89 seconds = a minute");
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  "eine Stunde",       "45 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  "eine Stunde",       "89 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  "2 Stunden",          "90 minutes = 2 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   "5 Stunden",          "5 hours = 5 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  "21 Stunden",         "21 hours = 21 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  "ein Tag",          "22 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  "ein Tag",          "35 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  "2 Tage",            "36 hours = 2 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   "ein Tag",          "1 day = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   "5 Tage",            "5 days = 5 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  "25 Tage",           "25 days = 25 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  "ein Monat",        "26 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  "ein Monat",        "30 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  "ein Monat",        "43 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  "2 Monate",          "46 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  "2 Monate",          "75 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  "3 Monate",          "76 days = 3 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   "ein Monat",        "1 month = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   "5 Monate",          "5 months = 5 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "ein Jahr",         "345 days = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 Jahre",           "548 days = 2 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "ein Jahr",         "1 year = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 Jahre",           "5 years = 5 years");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "in ein paar Sekunden", "prefix");
        test.equal(frozenMoment(0).from(30000), "vor ein paar Sekunden", "suffix");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "in ein paar Sekunden", "in a few seconds");
        test.equal(frozenMoment().add({d: 5}).fromNow(), "in 5 Tagen", "in 5 days");
        test.done();
    },
    ...
    "calendar day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     "Heute um 02:00 Uhr",   "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      "Heute um 02:25 Uhr",   "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       "Heute um 03:00 Uhr",   "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       "Morgen um 02:00 Uhr",  "tomorrow at the same time");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  "Heute um 01:00 Uhr",   "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  "Gestern um 02:00 Uhr", "yesterday at the same time");
        test.done();
    },
