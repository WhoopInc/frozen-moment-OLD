...
exports["lang:es"] = {
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "unos segundos", "44 seconds = a few seconds");
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "un año",        "345 days = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 años",       "548 days = 2 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "un año",        "1 year = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 años",       "5 years = 5 years");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "en unos segundos",  "prefix");
        test.equal(frozenMoment(0).from(30000), "hace unos segundos", "suffix");
        test.done();
    },

    "now from now" : function (test) {
        test.equal(frozenMoment().fromNow(), "hace unos segundos",  "now from now should display as in the past");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "en unos segundos", "en unos segundos");
    ...
    "calendar day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                         "hoy a las 2:00",     "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),          "hoy a las 2:25",     "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),           "hoy a las 3:00",     "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),           "mañana a las 2:00",  "tomorrow at the same time");
        test.equal(frozenMoment(a).add({d: 1, h : -1}).calendar(),   "mañana a la 1:00",   "tomorrow minus 1 hour");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),      "hoy a la 1:00",      "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),      "ayer a las 2:00",    "yesterday at the same time");
        test.done();
    },
