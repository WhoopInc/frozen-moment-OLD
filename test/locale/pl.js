...
exports["locale:pl"] = {
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "kilka sekund",  "44 seconds = a few seconds");
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  "44 minuty",     "44 minutes = 44 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  "godzina",       "45 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  "godzina",       "89 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  "2 godziny",     "90 minutes = 2 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   "5 godzin",      "5 hours = 5 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  "21 godzin",     "21 hours = 21 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  "1 dzień",       "22 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  "1 dzień",       "35 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  "2 dni",         "36 hours = 2 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   "1 dzień",       "1 day = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   "5 dni",         "5 days = 5 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  "25 dni",        "25 days = 25 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  "miesiąc",       "26 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  "miesiąc",       "30 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  "miesiąc",       "43 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  "2 miesiące",    "46 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  "2 miesiące",    "75 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  "3 miesiące",    "76 days = 3 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   "miesiąc",       "1 month = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   "5 miesięcy",    "5 months = 5 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "rok",           "345 days = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 lata",        "548 days = 2 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "rok",           "1 year = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 lat",         "5 years = 5 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 112}), true), "112 lat",       "112 years = 112 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 122}), true), "122 lata",      "122 years = 122 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 213}), true), "213 lat",       "213 years = 213 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 223}), true), "223 lata",      "223 years = 223 years");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "za kilka sekund",  "prefix");
        test.equal(frozenMoment(0).from(30000), "kilka sekund temu", "suffix");
        test.done();
    },

    "now from now" : function (test) {
        test.equal(frozenMoment().fromNow(), "kilka sekund temu",  "now from now should display as in the past");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "za kilka sekund", "in a few seconds");
        test.equal(frozenMoment().add({h: 1}).fromNow(), "za godzinę", "in an hour");
        test.equal(frozenMoment().add({d: 5}).fromNow(), "za 5 dni", "in 5 days");
        test.done();
    },
    ...
    "calendar day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     "Dziś o 02:00",     "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      "Dziś o 02:25",     "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       "Dziś o 03:00",     "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       "Jutro o 02:00",  "tomorrow at the same time");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  "Dziś o 01:00",     "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  "Wczoraj o 02:00",     "yesterday at the same time");
        test.done();
    },
