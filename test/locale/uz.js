...
exports["locale:uz"] = {
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "фурсат", "44 секунд = фурсат");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  "бир дакика",      "45 секунд = бир дакика");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  "бир дакика",      "89 секунд = бир дакика");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  "2 дакика",     "90 секунд = 2 дакика");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  "44 дакика",    "44 дакика = 44 дакика");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  "бир соат",       "45 минут = бир соат");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  "бир соат",       "89 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  "2 соат",       "90 минут = 2 соат");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   "5 соат",       "5 соат = 5 соат");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  "21 соат",      "21 соат = 21 соат");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  "бир кун",         "22 соат = бир кун");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  "бир кун",         "35 соат = бир кун");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  "2 кун",        "36 соат = 2 кун");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   "бир кун",         "1 кун = 1 кун");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   "5 кун",        "5 кун = 5 кун");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  "25 кун",       "25 кун = 25 кун");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  "бир ой",       "26 кун = бир ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  "бир ой",       "30 кун = бир ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  "бир ой",       "45 кун = бир ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  "2 ой",      "46 кун = 2 ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  "2 ой",      "75 кун = 2 ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  "3 ой",      "76 кун = 3 ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   "бир ой",       "бир ой = бир ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   "5 ой",      "5 ой = 5 ой");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "бир йил",        "345 кун = бир йил");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 йил",       "548 кун = 2 йил");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "бир йил",        "1 йил = бир йил");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 йил",       "5 йил = 5 йил");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "Якин фурсат ичида",  "prefix");
        test.equal(frozenMoment(0).from(30000), "Бир неча фурсат олдин", "suffix");
        test.done();
    },

    "now from now" : function (test) {
        test.equal(frozenMoment().fromNow(), "Бир неча фурсат олдин",  "now from now should display as in the past");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "Якин фурсат ичида", "in a few seconds");
        test.equal(frozenMoment().add({d: 5}).fromNow(), "Якин 5 кун ичида", "in 5 days");
        test.done();
    },
    ...
    "calendar day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     "Бугун соат 02:00 да",      "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      "Бугун соат 02:25 да",      "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       "Бугун соат 03:00 да",      "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       "Эртага 02:00 да",   "tomorrow at the same time");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  "Бугун соат 01:00 да",      "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  "Кеча соат 02:00 да",  "yesterday at the same time");
        test.done();
    },
    ...
    "weeks year starting sunday" : function (test) {
        test.equal(frozenMoment([2012, 0, 1]).week(), 1, "Jan  1 2012 should be week 52");
        test.equal(frozenMoment([2012, 0, 2]).week(),  2, "Jan  2 2012 should be week 1");
        test.equal(frozenMoment([2012, 0, 8]).week(),  2, "Jan  8 2012 should be week 1");
        test.equal(frozenMoment([2012, 0, 9]).week(),  3, "Jan  9 2012 should be week 2");
        test.equal(frozenMoment([2012, 0, 15]).week(), 3, "Jan 15 2012 should be week 2");
    ...
    "weeks year starting friday" : function (test) {
        test.equal(frozenMoment([2009, 11, 28]).week(), 1, "Dec 28 2009 should be week 53");
        test.equal(frozenMoment([2010,  0,  1]).week(), 1, "Jan  1 2010 should be week 53");
        test.equal(frozenMoment([2010,  0,  3]).week(), 1, "Jan  3 2010 should be week 53");
        test.equal(frozenMoment([2010,  0,  4]).week(),  2, "Jan  4 2010 should be week 1");
        test.equal(frozenMoment([2010,  0, 10]).week(),  2, "Jan 10 2010 should be week 1");
        test.equal(frozenMoment([2010,  0, 11]).week(),  3, "Jan 11 2010 should be week 2");

        test.done();
    },

    "weeks year starting saturday" : function (test) {
        test.equal(frozenMoment([2010, 11, 27]).week(), 1, "Dec 27 2010 should be week 52");
        test.equal(frozenMoment([2011,  0,  1]).week(), 1, "Jan  1 2011 should be week 52");
        test.equal(frozenMoment([2011,  0,  2]).week(), 1, "Jan  2 2011 should be week 52");
        test.equal(frozenMoment([2011,  0,  3]).week(),  2, "Jan  3 2011 should be week 1");
        test.equal(frozenMoment([2011,  0,  9]).week(),  2, "Jan  9 2011 should be week 1");
        test.equal(frozenMoment([2011,  0, 10]).week(),  3, "Jan 10 2011 should be week 2");

        test.done();
    },

    "weeks year starting sunday formatted" : function (test) {
        test.equal(frozenMoment([2012, 0,  1]).format('w ww wo'), '1 01 1', "Jan  1 2012 should be week 52");
        test.equal(frozenMoment([2012, 0,  2]).format('w ww wo'),   '2 02 2', "Jan  2 2012 should be week 1");
        test.equal(frozenMoment([2012, 0,  8]).format('w ww wo'),   '2 02 2', "Jan  8 2012 should be week 1");
        test.equal(frozenMoment([2012, 0,  9]).format('w ww wo'),   '3 03 3', "Jan  9 2012 should be week 2");
        test.equal(frozenMoment([2012, 0, 15]).format('w ww wo'),   '3 03 3', "Jan 15 2012 should be week 2");
