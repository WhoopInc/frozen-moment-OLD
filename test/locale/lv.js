...
exports['locale:lv'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'dažas sekundes',    '44 seconds = seconds');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  'minūti',      '45 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  'minūti',      '89 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  '2 minūtes',     '90 seconds = 2 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  '44 minūtes',    '44 minutes = 44 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  'stundu',       '45 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  'stundu',       '89 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  '2 stundas',       '90 minutes = 2 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   '5 stundas',       '5 hours = 5 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  '21 stunda',      '21 hours = 21 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  'dienu',         '22 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  'dienu',         '35 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 dienas',        '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'dienu',         '1 day = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   '5 dienas',        '5 days = 5 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  '25 dienas',       '25 days = 25 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  'mēnesi',       '26 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  'mēnesi',       '30 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  'mēnesi',       '43 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  '2 mēneši',      '46 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  '2 mēneši',      '75 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  '3 mēneši',      '76 days = 3 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   'mēnesi',       '1 month = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 mēneši',      '5 months = 5 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), 'gadu',        '345 days = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), '2 gadi',       '548 days = 2 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   'gadu',        '1 year = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   '5 gadi',       '5 years = 5 years');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'dažas sekundes vēlāk',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'dažas sekundes agrāk', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'dažas sekundes agrāk',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'dažas sekundes vēlāk', 'in seconds');
        test.equal(frozenMoment().add({d: 5}).fromNow(), '5 dienas vēlāk', 'in 5 days');
        test.done();
    },
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     'Šodien pulksten 02:00',     'today at the same time');
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      'Šodien pulksten 02:25',     'Now plus 25 min');
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       'Šodien pulksten 03:00',     'Now plus 1 hour');
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       'Rīt pulksten 02:00',  'tomorrow at the same time');
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  'Šodien pulksten 01:00',     'Now minus 1 hour');
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  'Vakar pulksten 02:00', 'yesterday at the same time');
        test.done();
    },
