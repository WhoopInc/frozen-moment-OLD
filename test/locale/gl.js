...
exports['locale:gl'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'uns segundos', '44 seconds = a few seconds');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  'unha hora',       '45 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  'unha hora',       '89 minutes = an hour');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  'un día',         '22 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  'un día',         '35 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 días',        '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'un día',         '1 day = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   '5 días',        '5 days = 5 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  '25 días',       '25 days = 25 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  'un mes',       '26 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  'un mes',       '30 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  'un mes',       '43 days = a month');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   'un mes',       '1 month = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 meses',      '5 months = 5 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), 'un ano',        '345 days = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), '2 anos',       '548 days = 2 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   'un ano',        '1 year = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   '5 anos',       '5 years = 5 years');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'nuns segundos',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'hai uns segundos', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'hai uns segundos',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'nuns segundos', 'en unos segundos');
        test.equal(frozenMoment().add({d: 5}).fromNow(), 'en 5 días', 'en 5 días');
        test.done();
    },
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                         'hoxe ás 2:00',     'today at the same time');
        test.equal(frozenMoment(a).add({m: 25}).calendar(),          'hoxe ás 2:25',     'Now plus 25 min');
        test.equal(frozenMoment(a).add({h: 1}).calendar(),           'hoxe ás 3:00',     'Now plus 1 hour');
        test.equal(frozenMoment(a).add({d: 1}).calendar(),           'mañá ás 2:00',  'tomorrow at the same time');
        test.equal(frozenMoment(a).add({d: 1, h : -1}).calendar(),   'mañá á 1:00',   'tomorrow minus 1 hour');
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),      'hoxe á 1:00',      'Now minus 1 hour');
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),      'onte á 2:00',    'yesterday at the same time');
        test.done();
    },
    ...
    'regression tests' : function (test) {
        var lastWeek = frozenMoment().subtract({d: 4}).hours(1);
    ...
    'weeks year starting sunday formatted' : function (test) {
        test.equal(frozenMoment([2011, 11, 26]).format('w ww wo'), '1 01 1º', 'Dec 26 2011 should be week 1');
        test.equal(frozenMoment([2012,  0,  1]).format('w ww wo'), '1 01 1º', 'Jan  1 2012 should be week 1');
        test.equal(frozenMoment([2012,  0,  2]).format('w ww wo'), '2 02 2º', 'Jan  2 2012 should be week 2');
        test.equal(frozenMoment([2012,  0,  8]).format('w ww wo'), '2 02 2º', 'Jan  8 2012 should be week 2');
        test.equal(frozenMoment([2012,  0,  9]).format('w ww wo'), '3 03 3º', 'Jan  9 2012 should be week 3');
