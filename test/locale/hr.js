...
exports['locale:hr'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'par sekundi', '44 seconds = a few seconds');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  'jedna minuta',   '45 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  'jedna minuta',   '89 seconds = a minute');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  'mjesec',     '26 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  'mjesec',     '30 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  'mjesec',     '43 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  '2 mjeseca',     '46 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  '2 mjeseca',     '75 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  '3 mjeseca',     '76 days = 3 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   'mjesec',     '1 month = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 mjeseci',    '5 months = 5 months');
    ...
    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'za par sekundi',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'prije par sekundi', 'prefix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'prije par sekundi',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'za par sekundi', 'in a few seconds');
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  'jučer u 2:00', 'yesterday at the same time');
        test.done();
    },
