...
exports['locale:nn'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'nokre sekund', '44 sekunder = a few seconds');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  'eit minutt',   '45 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  'eit minutt',   '89 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  '2 minutt',     '90 seconds = 2 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  '44 minutt',    '44 minutes = 44 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  'ein time',     '45 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  'ein time',     '89 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  '2 timar',      '90 minutes = 2 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   '5 timar',      '5 hours = 5 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  '21 timar',     '21 hours = 21 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  'ein dag',      '22 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  'ein dag',      '35 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 dagar',      '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'ein dag',      '1 day = a day');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  'ein månad',    '26 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  'ein månad',    '30 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  'ein månad',    '43 days = a month');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   'ein månad',    '1 month = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 månader',    '5 months = 5 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), 'eit år',       '345 days = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), '2 år',         '548 days = 2 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   'eit år',       '1 year = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   '5 år',         '5 years = 5 years');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'om nokre sekund',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'for nokre sekund sidan', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'for nokre sekund sidan',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'om nokre sekund', 'in a few seconds');
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     'I dag klokka 02:00',     'today at the same time');
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      'I dag klokka 02:25',     'Now plus 25 min');
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       'I dag klokka 03:00',     'Now plus 1 hour');
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       'I morgon klokka 02:00',  'tomorrow at the same time');
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  'I dag klokka 01:00',     'Now minus 1 hour');
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  'I går klokka 02:00',     'yesterday at the same time');
        test.done();
    },
