...
exports["locale:is"] = {
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "nokkrar sekúndur", "44 seconds = a few seconds");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  "mínúta",      "45 seconds = a minute");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  "mínúta",      "89 seconds = a minute");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  "2 mínútur",     "90 seconds = 2 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  "44 mínútur",    "44 minutes = 44 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 21}), true),  "21 mínúta",    "21 minutes = 21 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  "klukkustund",       "45 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  "klukkustund",       "89 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  "2 klukkustundir",       "90 minutes = 2 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   "5 klukkustundir",       "5 hours = 5 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  "21 klukkustund",      "21 hours = 21 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  "dagur",         "22 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  "dagur",         "35 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  "2 dagar",        "36 hours = 2 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   "dagur",         "1 day = a day");
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 11}), true),  "11 dagar",       "11 days = 11 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 21}), true),  "21 dagur",       "21 days = 21 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  "mánuður",       "26 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  "mánuður",       "30 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  "mánuður",       "43 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  "2 mánuðir",      "46 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  "2 mánuðir",      "75 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  "3 mánuðir",      "76 days = 3 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   "mánuður",       "1 month = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   "5 mánuðir",      "5 months = 5 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "ár",        "345 days = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 ár",       "548 days = 2 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "ár",        "1 year = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 ár",       "5 years = 5 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 21}), true),  "21 ár",       "21 years = 21 years");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "eftir nokkrar sekúndur",  "prefix");
        test.equal(frozenMoment(0).from(30000), "fyrir nokkrum sekúndum síðan", "suffix");
        test.equal(frozenMoment().subtract({m: 1}).fromNow(), "fyrir mínútu síðan", "a minute ago");
        test.done();
    },

    "now from now" : function (test) {
        test.equal(frozenMoment().fromNow(), "fyrir nokkrum sekúndum síðan",  "now from now should display as in the past");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "eftir nokkrar sekúndur", "in a few seconds");
        test.equal(frozenMoment().add({m: 1}).fromNow(), "eftir mínútu", "in a minute");
        test.equal(frozenMoment().add({d: 5}).fromNow(), "eftir 5 daga", "in 5 days");
        test.done();
    },
    ...
    "calendar day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     "í dag kl. 2:00",     "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      "í dag kl. 2:25",     "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       "í dag kl. 3:00",     "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       "á morgun kl. 2:00",  "tomorrow at the same time");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  "í dag kl. 1:00",     "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  "í gær kl. 2:00", "yesterday at the same time");
        test.done();
    },
