...
exports["locale:tl-ph"] = {
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "ilocale segundo", "44 seconds = a few seconds");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  "isang minuto",   "45 seconds = a minute");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  "isang minuto",   "89 seconds = a minute");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  "2 minuto",  "90 seconds = 2 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  "44 minuto", "44 minutes = 44 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  "isang oras",    "45 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  "isang oras",    "89 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  "2 oras",    "90 minutes = 2 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   "5 oras",    "5 hours = 5 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  "21 oras",   "21 hours = 21 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  "isang araw",      "22 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  "isang araw",      "35 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  "2 araw",     "36 hours = 2 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   "isang araw",      "1 day = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   "5 araw",     "5 days = 5 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  "25 araw",    "25 days = 25 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  "isang buwan",    "26 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  "isang buwan",    "30 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  "isang buwan",    "43 days = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  "2 buwan",   "46 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  "2 buwan",   "75 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  "3 buwan",   "76 days = 3 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   "isang buwan",    "1 month = a month");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   "5 buwan",   "5 months = 5 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "isang taon",     "345 days = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 taon",    "548 days = 2 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "isang taon",     "1 year = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 taon",    "5 years = 5 years");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "sa loob ng ilocale segundo", "prefix");
        test.equal(frozenMoment(0).from(30000), "ilocale segundo ang nakalipas", "suffix");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "sa loob ng ilocale segundo", "in a few seconds");
        test.equal(frozenMoment().add({d: 5}).fromNow(), "sa loob ng 5 araw", "in 5 days");
        test.done();
    },
    ...
    "same day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     "Ngayon sa 02:00",    "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      "Ngayon sa 02:25",    "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       "Ngayon sa 03:00",    "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       "Bukas sa 02:00",     "tomorrow at the same time");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  "Ngayon sa 01:00",    "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  "Kahapon sa 02:00",   "yesterday at the same time");
        test.done();
    },
