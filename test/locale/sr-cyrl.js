...
exports["locale:sr-cyrl"] = {
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "неколико секунди", "44 seconds = a few seconds");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  "један минут",   "45 seconds = a minute");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  "један минут",   "89 seconds = a minute");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  "2 минуте",     "90 seconds = 2 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  "44 минута",     "44 minutes = 44 minutes");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  "један сат",      "45 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  "један сат",      "89 minutes = an hour");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  "2 сата",        "90 minutes = 2 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   "5 сати",         "5 hours = 5 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  "21 сати",        "21 hours = 21 hours");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  "дан",       "22 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  "дан",       "35 hours = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  "2 дана",        "36 hours = 2 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   "дан",       "1 day = a day");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   "5 дана",        "5 days = 5 days");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  "25 дана",       "25 days = 25 days");
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  "2 месеца",     "46 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  "2 месеца",     "75 days = 2 months");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  "3 месеца",     "76 days = 3 months");
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "годину",     "345 days = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 године",       "548 days = 2 years");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "годину",     "1 year = a year");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 година",        "5 years = 5 years");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "за неколико секунди",  "prefix");
        test.equal(frozenMoment(0).from(30000), "пре неколико секунди", "prefix");
        test.done();
    },

    "now from now" : function (test) {
        test.equal(frozenMoment().fromNow(), "пре неколико секунди",  "now from now should display as in the past");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "за неколико секунди", "in a few seconds");
        test.equal(frozenMoment().add({d: 5}).fromNow(), "за 5 дана", "in 5 days");
        test.done();
    },
    ...
    "calendar day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     "данас у 2:00",  "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      "данас у 2:25",  "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       "данас у 3:00",  "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       "сутра у 2:00",  "tomorrow at the same time");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  "данас у 1:00",  "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  "јуче у 2:00", "yesterday at the same time");
        test.done();
    },
