...
exports['locale:ro'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'câteva secunde', '44 seconds = a few seconds');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  'un minut',       '45 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  'un minut',       '89 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  '2 minute',       '90 seconds = 2 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  '44 de minute',   '44 minutes = 44 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  'o oră',          '45 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  'o oră',          '89 minutes = an hour');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  '21 de ore',      '21 hours = 21 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  'o zi',           '22 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  'o zi',           '35 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 zile',         '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'o zi',           '1 day = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   '5 zile',         '5 days = 5 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  '25 de zile',     '25 days = 25 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  'o lună',         '26 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  'o lună',         '30 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  'o lună',         '43 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  '2 luni',         '46 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  '2 luni',         '75 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  '3 luni',         '76 days = 3 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   'o lună',         '1 month = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 luni',         '5 months = 5 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), 'un an',          '345 days = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), '2 ani',          '548 days = 2 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   'un an',          '1 year = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   '5 ani',          '5 years = 5 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 19}), true),   '19 ani',        '19 years = 19 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 20}), true),   '20 de ani',     '20 years = 20 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 100}), true),   '100 de ani',   '100 years = 100 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 101}), true),   '101 ani',      '101 years = 101 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 119}), true),   '119 ani',      '119 years = 119 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 120}), true),   '120 de ani',   '120 years = 120 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 219}), true),   '219 ani',      '219 years = 219 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 220}), true),   '220 de ani',   '220 years = 220 years');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'peste câteva secunde',   'prefix');
        test.equal(frozenMoment(0).from(30000), 'câteva secunde în urmă', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'câteva secunde în urmă',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'peste câteva secunde', 'in a few seconds');
        test.equal(frozenMoment().add({d: 5}).fromNow(), 'peste 5 zile', 'in 5 days');
        test.done();
    },
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     'azi la 2:00',     'today at the same time');
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      'azi la 2:25',     'Now plus 25 min');
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       'azi la 3:00',     'Now plus 1 hour');
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       'mâine la 2:00',   'tomorrow at the same time');
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  'azi la 1:00',     'Now minus 1 hour');
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  'ieri la 2:00',    'yesterday at the same time');
        test.done();
    },
    ...
    'weeks year starting sunday formatted' : function (test) {
        test.equal(frozenMoment([2011, 11, 26]).format('w ww wo'), '1 01 1', 'Dec 26 2011 should be week 1');
        test.equal(frozenMoment([2012,  0,  1]).format('w ww wo'), '1 01 1', 'Jan  1 2012 should be week 1');
        test.equal(frozenMoment([2012,  0,  2]).format('w ww wo'), '2 02 2', 'Jan  2 2012 should be week 2');
        test.equal(frozenMoment([2012,  0,  8]).format('w ww wo'), '2 02 2', 'Jan  8 2012 should be week 2');
        test.equal(frozenMoment([2012,  0,  9]).format('w ww wo'), '3 03 3', 'Jan  9 2012 should be week 3');
