...
exports['locale:bg'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'няколко секунди', '44 seconds = a few seconds');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 дни',           '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'ден',             '1 day = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   '5 дни',           '5 days = 5 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  '25 дни',          '25 days = 25 days');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 месеца',        '5 months = 5 months');
    ...
    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'след няколко секунди',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'преди няколко секунди', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'преди няколко секунди',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'след няколко секунди', 'in a few seconds');
        test.equal(frozenMoment().add({d: 5}).fromNow(), 'след 5 дни', 'in 5 days');
        test.done();
    },
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     'Днес в 2:00',  'today at the same time');
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      'Днес в 2:25',  'Now plus 25 min');
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       'Днес в 3:00',  'Now plus 1 hour');
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       'Утре в 2:00',  'tomorrow at the same time');
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  'Днес в 1:00',  'Now minus 1 hour');
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  'Вчера в 2:00', 'yesterday at the same time');
        test.done();
    },
