...
exports["locale:km"] = {
    ...
    "from": function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true), "ប៉ុន្មានវិនាទី", "44 seconds = ប៉ុន្មានវិនាទី");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true), "មួយនាទី", "45 seconds = មួយនាទី");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true), "មួយនាទី", "89 seconds = មួយនាទី");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true), "2 នាទី", "90 seconds = 2 នាទី");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true), "44 នាទី", "44 minutes = 44 នាទី");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true), "មួយម៉ោង", "45 minutes = មួយម៉ោង");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true), "មួយម៉ោង", "89 minutes = មួយម៉ោង");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true), "2 ម៉ោង", "90 minutes = 2 ម៉ោង");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true), "5 ម៉ោង", "5 hours = 5 ម៉ោង");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true), "21 ម៉ោង", "21 hours = 21 ម៉ោង");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true), "មួយថ្ងៃ", "22 hours = មួយថ្ងៃ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true), "មួយថ្ងៃ", "35 hours = មួយថ្ងៃ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true), "2 ថ្ងៃ", "36 hours = 2 ថ្ងៃ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true), "មួយថ្ងៃ", "1 day = មួយថ្ងៃ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true), "5 ថ្ងៃ", "5 days = 5 ថ្ងៃ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true), "25 ថ្ងៃ", "25 days = 25 ថ្ងៃ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true), "មួយខែ", "26 days = មួយខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true), "មួយខែ", "30 days = មួយខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true), "មួយខែ", "43 days = មួយខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true), "2 ខែ", "46 days = 2 ខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true), "2 ខែ", "75 days = 2 ខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true), "3 ខែ", "76 days = 3 ខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true), "មួយខែ", "1 month = មួយខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true), "5 ខែ", "5 months = 5 ខែ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "មួយឆ្នាំ", "345 days = មួយឆ្នាំ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 ឆ្នាំ", "548 days = 2 ឆ្នាំ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true), "មួយឆ្នាំ", "1 year = មួយឆ្នាំ");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true), "5 ឆ្នាំ", "5 years = 5 ឆ្នាំ");
        test.done();
    },

    "suffix": function (test) {
        test.equal(frozenMoment(30000).from(0), "ប៉ុន្មានវិនាទីទៀត", "prefix");
        test.equal(frozenMoment(0).from(30000), "ប៉ុន្មានវិនាទីមុន", "suffix");
        test.done();
    },

    "now from now": function (test) {
        test.equal(frozenMoment().fromNow(), "ប៉ុន្មានវិនាទីមុន", "now from now should display as in the past");
        test.done();
    },

    "fromNow": function (test) {
        test.equal(frozenMoment().add({
            s: 30
    ...
    "calendar day": function (test) {
        ...
        test.equal(frozenMoment(a).calendar(), "ថ្ងៃនៈ ម៉ោង 02:00", "today at the same time");
        test.equal(frozenMoment(a).add({
            m: 25
        ...
        test.equal(frozenMoment(a).subtract({
            h: 1
    ...
    "calendar all else": function (test) {
        var weeksAgo = frozenMoment().subtract({
                w: 1
            }),
            weeksFromNow = frozenMoment().add({
                w: 1
            });
        ...
        weeksAgo = frozenMoment().subtract({
            w: 2
        });
        weeksFromNow = frozenMoment().add({
            w: 2
