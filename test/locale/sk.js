...
exports['locale:sk'] = {
    ...
    'parse' : function (test) {
        ...
        function equalTest(input, mmm, monthIndex) {
            test.equal(frozenMoment(input, mmm).month(), monthIndex, input + ' should be month ' + (monthIndex + 1));
        }
        for (i = 0; i < 12; i++) {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'pár sekúnd',  '44 seconds = a few seconds');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  'minúta',        '45 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  'minúta',        '89 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  '2 minúty',      '90 seconds = 2 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  '44 minút',     '44 minutes = 44 minutes');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  'hodina',       '45 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  'hodina',       '89 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  '2 hodiny',     '90 minutes = 2 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   '5 hodín',      '5 hours = 5 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  '21 hodín',     '21 hours = 21 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  'deň',       '22 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  'deň',       '35 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 dni',         '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'deň',       '1 day = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   '5 dní',         '5 days = 5 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  '25 dní',        '25 days = 25 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  'mesiac',       '26 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  'mesiac',       '30 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  'mesiac',       '43 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  '2 mesiace',    '46 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  '2 mesiace',    '75 days = 2 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  '3 mesiace',    '76 days = 3 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   'mesiac',       '1 month = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 mesiacov',    '5 months = 5 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), 'rok',           '345 days = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), '2 roky',        '548 days = 2 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   'rok',           '1 year = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   '5 rokov',         '5 years = 5 years');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'za pár sekúnd',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'pred pár sekundami', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'pred pár sekundami',  'now from now should display as in the past');
        test.done();
    },

    'fromNow (future)' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'za pár sekúnd', 'in a few seconds');
        test.equal(frozenMoment().add({m: 1}).fromNow(), 'za minútu', 'in a minute');
        test.equal(frozenMoment().add({m: 3}).fromNow(), 'za 3 minúty', 'in 3 minutes');
        test.equal(frozenMoment().add({m: 10}).fromNow(), 'za 10 minút', 'in 10 minutes');
        test.equal(frozenMoment().add({h: 1}).fromNow(), 'za hodinu', 'in an hour');
        test.equal(frozenMoment().add({h: 3}).fromNow(), 'za 3 hodiny', 'in 3 hours');
        test.equal(frozenMoment().add({h: 10}).fromNow(), 'za 10 hodín', 'in 10 hours');
        test.equal(frozenMoment().add({d: 1}).fromNow(), 'za deň', 'in a day');
        test.equal(frozenMoment().add({d: 3}).fromNow(), 'za 3 dni', 'in 3 days');
        test.equal(frozenMoment().add({d: 10}).fromNow(), 'za 10 dní', 'in 10 days');
        test.equal(frozenMoment().add({M: 1}).fromNow(), 'za mesiac', 'in a month');
        test.equal(frozenMoment().add({M: 3}).fromNow(), 'za 3 mesiace', 'in 3 months');
        test.equal(frozenMoment().add({M: 10}).fromNow(), 'za 10 mesiacov', 'in 10 months');
        test.equal(frozenMoment().add({y: 1}).fromNow(), 'za rok', 'in a year');
        test.equal(frozenMoment().add({y: 3}).fromNow(), 'za 3 roky', 'in 3 years');
        test.equal(frozenMoment().add({y: 10}).fromNow(), 'za 10 rokov', 'in 10 years');
        test.done();
    },

    'fromNow (past)' : function (test) {
        test.equal(frozenMoment().subtract({s: 30}).fromNow(), 'pred pár sekundami', 'a few seconds ago');
        test.equal(frozenMoment().subtract({m: 1}).fromNow(), 'pred minútou', 'a minute ago');
        test.equal(frozenMoment().subtract({m: 3}).fromNow(), 'pred 3 minútami', '3 minutes ago');
        test.equal(frozenMoment().subtract({m: 10}).fromNow(), 'pred 10 minútami', '10 minutes ago');
        test.equal(frozenMoment().subtract({h: 1}).fromNow(), 'pred hodinou', 'an hour ago');
        test.equal(frozenMoment().subtract({h: 3}).fromNow(), 'pred 3 hodinami', '3 hours ago');
        test.equal(frozenMoment().subtract({h: 10}).fromNow(), 'pred 10 hodinami', '10 hours ago');
        test.equal(frozenMoment().subtract({d: 1}).fromNow(), 'pred dňom', 'a day ago');
        test.equal(frozenMoment().subtract({d: 3}).fromNow(), 'pred 3 dňami', '3 days ago');
        test.equal(frozenMoment().subtract({d: 10}).fromNow(), 'pred 10 dňami', '10 days ago');
        test.equal(frozenMoment().subtract({M: 1}).fromNow(), 'pred mesiacom', 'a month ago');
        test.equal(frozenMoment().subtract({M: 3}).fromNow(), 'pred 3 mesiacmi', '3 months ago');
        test.equal(frozenMoment().subtract({M: 10}).fromNow(), 'pred 10 mesiacmi', '10 months ago');
        test.equal(frozenMoment().subtract({y: 1}).fromNow(), 'pred rokom', 'a year ago');
        test.equal(frozenMoment().subtract({y: 3}).fromNow(), 'pred 3 rokmi', '3 years ago');
        test.equal(frozenMoment().subtract({y: 10}).fromNow(), 'pred 10 rokmi', '10 years ago');
        test.done();
    },
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     'dnes o 2:00',     'today at the same time');
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      'dnes o 2:25',     'Now plus 25 min');
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       'dnes o 3:00',     'Now plus 1 hour');
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       'zajtra o 2:00',  'tomorrow at the same time');
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  'dnes o 1:00',     'Now minus 1 hour');
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  'včera o 2:00',     'yesterday at the same time');
        test.done();
    },
    ...
    'humanize duration' : function (test) {
        test.equal(frozenMoment.duration(1, 'minutes').humanize(), 'minúta', 'a minute (future)');
        test.equal(frozenMoment.duration(1, 'minutes').humanize(true), 'za minútu', 'in a minute');
        test.equal(frozenMoment.duration(-1, 'minutes').humanize(), 'minúta', 'a minute (past)');
        test.equal(frozenMoment.duration(-1, 'minutes').humanize(true), 'pred minútou', 'a minute ago');
        test.done();
    },
