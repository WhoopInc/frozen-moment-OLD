...
exports["locale:hu"] = {
    ...
    "meridiem" : function (test) {
        test.equal(frozenMoment([2011, 2, 23,  0,  0]).format('a'), "de", "am");
        test.equal(frozenMoment([2011, 2, 23, 11, 59]).format('a'), "de", "am");
        test.equal(frozenMoment([2011, 2, 23, 12,  0]).format('a'), "du", "pm");
        test.equal(frozenMoment([2011, 2, 23, 23, 59]).format('a'), "du", "pm");
    ...
    "from" : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  "néhány másodperc", "44 másodperc = néhány másodperc");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  "egy perc",         "45 másodperc = egy perc");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  "egy perc",         "89 másodperc = egy perc");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 90}), true),  "2 perc",           "90 másodperc = 2 perc");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 44}), true),  "44 perc",          "44 perc = 44 perc");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  "egy óra",          "45 perc = egy óra");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  "egy óra",          "89 perc = egy óra");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  "2 óra",            "90 perc = 2 óra");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   "5 óra",            "5 óra = 5 óra");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  "21 óra",           "21 óra = 21 óra");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  "egy nap",          "22 óra = egy nap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  "egy nap",          "35 óra = egy nap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  "2 nap",            "36 óra = 2 nap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   "egy nap",          "1 nap = egy nap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   "5 nap",            "5 nap = 5 nap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  "25 nap",           "25 nap = 25 nap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  "egy hónap",        "26 nap = egy hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  "egy hónap",        "30 nap = egy hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  "egy hónap",        "45 nap = egy hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 46}), true),  "2 hónap",          "46 nap = 2 hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 74}), true),  "2 hónap",          "75 nap = 2 hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  "3 hónap",          "76 nap = 3 hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   "egy hónap",        "1 hónap = egy hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   "5 hónap",          "5 hónap = 5 hónap");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), "egy év",           "345 nap = egy év");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), "2 év",             "548 nap = 2 év");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   "egy év",           "1 év = egy év");
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   "5 év",             "5 év = 5 év");
        test.done();
    },

    "suffix" : function (test) {
        test.equal(frozenMoment(30000).from(0), "néhány másodperc múlva",  "prefix");
        test.equal(frozenMoment(0).from(30000), "néhány másodperce", "suffix");
        test.done();
    },

    "now from now" : function (test) {
        test.equal(frozenMoment().fromNow(), "néhány másodperce",  "now from now should display as in the past");
        test.done();
    },

    "fromNow" : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), "néhány másodperc múlva", "néhány másodperc múlva");
        test.equal(frozenMoment().add({d: 5}).fromNow(), "5 nap múlva", "5 nap múlva");
        test.done();
    },
    ...
    "calendar day" : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     "ma 2:00-kor",     "today at the same time");
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      "ma 2:25-kor",     "Now plus 25 min");
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       "ma 3:00-kor",     "Now plus 1 hour");
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       "holnap 2:00-kor", "tomorrow at the same time");
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  "ma 1:00-kor",     "Now minus 1 hour");
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  "tegnap 2:00-kor", "yesterday at the same time");
        test.done();
    },
