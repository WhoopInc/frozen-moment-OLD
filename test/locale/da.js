...
exports['locale:da'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'få sekunder', '44 seconds = a few seconds');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  'et minut',    '45 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  'et minut',    '89 seconds = a minute');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 dage',      '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'en dag',      '1 day = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 5}), true),   '5 dage',      '5 days = 5 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 25}), true),  '25 dage',     '25 days = 25 days');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), 'et år',       '345 days = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), '2 år',        '548 days = 2 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   'et år',       '1 year = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   '5 år',        '5 years = 5 years');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'om få sekunder',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'få sekunder siden', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'få sekunder siden',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'om få sekunder', 'in a few seconds');
        test.equal(frozenMoment().add({d: 5}).fromNow(), 'om 5 dage', 'in 5 days');
        test.done();
    },
