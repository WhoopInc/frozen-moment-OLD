...
exports['locale:sl'] = {
    ...
    'from' : function (test) {
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 44}), true),  'nekaj sekund', '44 seconds = a few seconds');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 45}), true),  'ena minuta',   '45 seconds = a minute');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({s: 89}), true),  'ena minuta',   '89 seconds = a minute');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 45}), true),  'ena ura',      '45 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 89}), true),  'ena ura',      '89 minutes = an hour');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({m: 90}), true),  '2 uri',        '90 minutes = 2 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 5}), true),   '5 ur',         '5 hours = 5 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 21}), true),  '21 ur',        '21 hours = 21 hours');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 22}), true),  'en dan',       '22 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 35}), true),  'en dan',       '35 hours = a day');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({h: 36}), true),  '2 dni',        '36 hours = 2 days');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 1}), true),   'en dan',       '1 day = a day');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 26}), true),  'en mesec',     '26 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 30}), true),  'en mesec',     '30 days = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 43}), true),  'en mesec',     '43 days = a month');
        ...
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 76}), true),  '3 mesece',     '76 days = 3 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 1}), true),   'en mesec',     '1 month = a month');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({M: 5}), true),   '5 mesecev',    '5 months = 5 months');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 345}), true), 'eno leto',     '345 days = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({d: 548}), true), '2 leti',       '548 days = 2 years');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 1}), true),   'eno leto',     '1 year = a year');
        test.equal(start.from(frozenMoment([2007, 1, 28]).add({y: 5}), true),   '5 let',        '5 years = 5 years');
        test.done();
    },

    'suffix' : function (test) {
        test.equal(frozenMoment(30000).from(0), 'čez nekaj sekund',  'prefix');
        test.equal(frozenMoment(0).from(30000), 'nekaj sekund nazaj', 'suffix');
        test.done();
    },

    'now from now' : function (test) {
        test.equal(frozenMoment().fromNow(), 'nekaj sekund nazaj',  'now from now should display as in the past');
        test.done();
    },

    'fromNow' : function (test) {
        test.equal(frozenMoment().add({s: 30}).fromNow(), 'čez nekaj sekund', 'in a few seconds');
        test.equal(frozenMoment().add({d: 5}).fromNow(), 'čez 5 dni', 'in 5 days');
        test.done();
    },
    ...
    'calendar day' : function (test) {
        ...
        test.equal(frozenMoment(a).calendar(),                     'danes ob 2:00',  'today at the same time');
        test.equal(frozenMoment(a).add({m: 25}).calendar(),      'danes ob 2:25',  'Now plus 25 min');
        test.equal(frozenMoment(a).add({h: 1}).calendar(),       'danes ob 3:00',  'Now plus 1 hour');
        test.equal(frozenMoment(a).add({d: 1}).calendar(),       'jutri ob 2:00',  'tomorrow at the same time');
        test.equal(frozenMoment(a).subtract({h: 1}).calendar(),  'danes ob 1:00',  'Now minus 1 hour');
        test.equal(frozenMoment(a).subtract({d: 1}).calendar(),  'včeraj ob 2:00', 'yesterday at the same time');
        test.done();
    },
